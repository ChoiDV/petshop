...
-- 찜 눌렀는지 안눌렀는지 여부 
SELECT COUNT(*) FROM DZIM WHERE MID='aaa' AND DNUM=7;

-- 찜 취소 
DELETE FROM DZIM WHERE MID='aaa' AND DNUM=11;
ROLLBACK;
-- 내가 찜한 리스트 
SELECT * FROM DZIM WHERE MID='aaa' ORDER BY ZDATE DESC;
COMMIT;

SELECT * FROM DOG WHERE DNUM IN(SELECT DNUM FROM DZIM WHERE MID='aaa') ORDER BY (SELECT ZDATE FROM DZIM);
...
SELECT DNUM FROM DZIM WHERE MID='aaa';

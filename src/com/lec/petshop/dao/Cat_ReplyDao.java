...
public class Cat_ReplyDao {
	...
	public ArrayList<Cat_ReplyDto> listCReply(int cnum, int startRow, int endRow) {
		...
		String sql = "SELECT C.* FROM(SELECT ROWNUM RN, A.* FROM "
				+ "                (SELECT * FROM CAT_REPLY WHERE CNUM= ? ORDER BY RDATE DESC) A) C "
	...
	public int updateCReply(String reply_content, String rip, int rno) {
		...
		Connection conn = null;
		PreparedStatement pstmt = null;
		String sql = "UPDATE CAT_REPLY SET REPLY_CONTENT= ? ," + "                        RIP = ? "
				+ "                            WHERE RNO= ? ";
		try {
			conn = getConnection();
			pstmt = conn.prepareStatement(sql);
	...
	// 고양이 댓글수 가져오기
	public int totalRCnt(int cnum) {
		int totalRCnt = 0;
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String sql = "SELECT COUNT(*) FROM CAT_REPLY WHERE CNUM= ?";
		try {
			conn = getConnection();
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, cnum);

...
public class Dog_ReplyDao {
	...
	// 특정 강아지 의 댓글 출력
	public ArrayList<Dog_ReplyDto> replyListDog(int dnum) {
		ArrayList<Dog_ReplyDto> dtos = new ArrayList<Dog_ReplyDto>();
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String sql = "SELECT * FROM DOG_REPLY WHERE DNUM= ?";
		try {
			conn = getConnection();
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, dnum);
			...
			while (rs.next()) {
				...
				Date rdate = rs.getDate("rdate");
				String rip = rs.getString("rip");
				dtos.add(new Dog_ReplyDto(0, rno, dnum, mid, reply_content, rdate, rip));
			}
		} catch (SQLException e) {
			System.out.println(e.getMessage() + dnum + " 번 댓글 출력");
		} finally {
	...
	// 특정 댓글 DTO 가져오기 
	public Dog_ReplyDto dogModifyReplyContent(int rno) {
		Dog_ReplyDto dto = null;
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String sql = "SELECT * FROM DOG_REPLY WHERE RNO= ?";
		try {
			conn = getConnection();
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, rno);
			...
			if (rs.next()) {				 
				...
				Date rdate = rs.getDate("rdate");
				String rip = rs.getString("rip");
				dto = new Dog_ReplyDto(rn, rno, dnum, mid, reply_content, rdate, rip);
			}
		} catch (Exception e) {

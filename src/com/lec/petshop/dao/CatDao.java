...
public class CatDao {
	...
	public ArrayList<CatDto> listCat(int startRow, int endRow) {
		...
		String sql = "SELECT * FROM(SELECT ROWNUM RN, A.* FROM"
				...
				+ "    WHERE C.CBREEDNO = CB.CBREEDNO" + "        AND RN BETWEEN ? AND ?" + "            ORDER BY RN ";
		try {
			conn = getConnection();
			pstmt = conn.prepareStatement(sql);
	...
	// chit 순으로 메인 페이지에 출력
	public ArrayList<CatDto> hitListCat() {
		ArrayList<CatDto> dtos = new ArrayList<CatDto>();
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	...
	public CatDto catContent(int cnum) {
		...
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String sql = "SELECT * FROM CAT C, CBREED CB" + "    WHERE C.CBREEDNO = CB.CBREEDNO" + "        AND CNUM= ? ";
		try {
			conn = getConnection();
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, cnum);
	...
	private void hitUp(int cnum) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		String sql = "UPDATE CAT SET CHIT= CHIT +1" + "   	 WHERE CNUM= ? ";
		try {
			conn = getConnection();
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, cnum);
	...
	public int updateCat(CatDto dto) {
		...
		Connection conn = null;
		PreparedStatement pstmt = null;
		String sql = "UPDATE CAT SET CNAME= ? ," + "                CGENDER= ? ," + "                 CBIRTH= ? ,"
				+ "                  CPRICE= ? ," + "                   CBREEDNO= ? ," + "                    AID= ? ,"
				+ "                     CCONTENT= ? ," + "                      CIMAGE1= ? ,"
				+ "                       CIMAGE2= ?," + "                        CIMAGE3= ? ,"
				+ "                         CIMAGE4= ? ," + "                          CIMAGE5= ? ,"
	...
	public int reservationCat(int cnum) {
		...
		Connection conn = null;
		PreparedStatement pstmt = null;
		String sql = "UPDATE CAT SET CR_CHECK=0 " + "    WHERE CNUM= ? ";
		try {
			conn = getConnection();
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, cnum);
	...
	public int cancellationCat(int cnum) {
		...
		Connection conn = null;
		PreparedStatement pstmt = null;
		String sql = "UPDATE CAT SET CR_CHECK=1 " + "    WHERE CNUM= ? ";
		try {
			conn = getConnection();
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, cnum);
